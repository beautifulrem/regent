...
**[打开线上演示 →](https://mandate-app-murex.vercel.app)**。什么都不用装,它打开的是**主网回放**:
一段录制好、完全自包含的 Base 主网真实运行(三跳委托链、Venice TEE 委员会、x402 过路费、1Shot
中继)。每个产物都真实、都链接到 Basescan,这是零配置路径,从这里开始即可。

而**实时 Base Sepolia 流程**(连你自己的钱包、授权一份受限委托、看 AI 投票、x402 过路费逐票结算、
再撤销)跑在**编排器**上:它是个有状态的服务,持有实时委托链与签名密钥,因此**不做托管**。你在本地
起它,应用通过 `http://localhost:8787` 与它通信;线上链接在探测不到编排器时会自动回落到回放。要自己
驱动实时流程:

```bash
git clone https://github.com/beautifulrem/regent && cd regent
pnpm install
pnpm bootstrap:accounts          # 一次性密钥写入 .env + 注资清单;给打印出的地址充值
...
然后打开 `http://localhost:3000`(或在同一台机器上打开线上链接,浏览器允许 `https` 页面访问
`http://localhost`),把 `.env` 里的 `USER_DEMO_PK` 导入 MetaMask,然后授权。过路费会逐票结算:
编排器在授权时给连入的智能账户铸足 mUSDC 的 x402 预算,所以刚连入的新钱包也能用。
...
的表现。上表"实时 Base Sepolia"那一列需要在本地起编排器(见上文 **▶️ 试用**);在线上链接里若探测
不到编排器,应用会改为打开主网回放。提示:打开
