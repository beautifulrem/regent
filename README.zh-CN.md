...
[![Demo video](https://img.shields.io/badge/演示视频-观看-ff0000?logo=youtube&logoColor=white)](https://www.youtube.com/watch?v=55HJ_LHQdqY)
...
<a href="https://www.youtube.com/watch?v=55HJ_LHQdqY">
  <img src="https://i.ytimg.com/vi/55HJ_LHQdqY/maxresdefault.jpg" width="640" alt="Regent 演示走查(YouTube):授权一份受限委托,Venice TEE 委员会决策,代理投票,x402 过路费结算,再一键撤销整条链">
</a>

**[打开线上演示 →](https://mandate-app-murex.vercel.app)**,整套流程在浏览器里就能跑通,无需安装、
也无需给钱包充值:连接任意钱包,授权一份受限的 ERC-7710 委托,看 Venice TEE 委员会决策、代理在
Base Sepolia 上铸票,看 x402 过路费逐票结算,再一键撤销整条链。应用已接到一个托管的编排器,所以谁
打开链接都能跑这条实时流程。把顶栏网络切到 **Base 主网**,即可看那段真实资金的录制运行。部署、gas、
撤销全部由赞助方承担,编排器还在授权时给连入的智能账户铸足 mUSDC 的 x402 预算,所以全新钱包什么都
不用准备。

想在本地自己跑整套(开发,或把应用指向你自己的编排器):

```bash
git clone https://github.com/beautifulrem/regent && cd regent
pnpm install
pnpm bootstrap:accounts          # 一次性密钥写入 .env + 注资清单
pnpm proposal --reseed --wait    # 开一个新的 300s Active 提案
pnpm --filter @mandate/orchestrator serve   # 编排器跑在 :8787
pnpm --filter @mandate/app dev              # 应用跑在 :3000
```
...
的表现。上表"实时 Base Sepolia"那条流程已接到托管的编排器,因此直接在线上链接里就能跑(也可以本地
自起一个,见上文 **▶️ 试用**)。提示:打开
