...
const en = {
  ...
  nodeTips: {
    you: 'Your MetaMask smart account. It signs ONE ERC-7710 delegation — vote-only, capped, expiring — and can kill the whole chain anytime with Recall.',
    orch: 'Agent #1 — holds your root delegation and re-delegates a NARROWER scope to the analyst. A child grant can only shrink authority, never grow it.',
    fiscal: 'TEE lens 1/4 — audits budget discipline: spend size, payout structure, treasury impact.',
    growth: 'TEE lens 2/4 — weighs ecosystem upside: builder value, adoption, long-term growth.',
    security: 'TEE lens 3/4 — hunts risk: attack surface, accountability gaps, fund safety.',
    participation: 'TEE lens 4/4 — checks governance health: community access, transparency, precedent.',
    synthesis: 'Agent #2 (the analyst) — weighs the four lens verdicts inside the Venice TEE and rules For / Against / Abstain. Its ruling is exactly what gets cast on-chain.',
    burner: 'A throwaway EOA upgraded to a smart account via EIP-7702 — holds 0 ETH, signs the 7710 bundle, and pays every fee in USDC only.',
    oneShot: 'The 1Shot permissionless relayer — fronts the real ETH gas and broadcasts the castVote bundle on Base mainnet, repaid in USDC.',
    board: 'The on-chain board that tallies every vote. The AI casts here AS your smart account — any wallet can join the same proposal.',
    venice: 'Venice AI — private inference inside an Intel TDX enclave (attestation verified); it can even speak the verdict via /audio/speech.',
  },
...
const zh: Dict = {
  ...
  nodeTips: {
    you: '你的 MetaMask 智能账户。只签一次 ERC-7710 委托——仅投票、有上限、会过期——且随时可用 Recall 杀掉整条授权链。',
    orch: '一号代理（编排器）——持有你的根委托，并以更窄的范围转授给分析师。子委托只能收窄权限，不能放大。',
    fiscal: 'TEE 视角 1/4——审预算纪律：开支规模、放款结构、财政影响。',
    growth: 'TEE 视角 2/4——看生态收益：开发者价值、采用度、长期增长。',
    security: 'TEE 视角 3/4——查风险：攻击面、问责缺口、资金安全。',
    participation: 'TEE 视角 4/4——看治理健康：社区参与、透明度、先例。',
    synthesis: '二号代理（分析师/终裁）——在 Venice TEE 内权衡四个视角的结论，裁定赞成/反对/弃权；该裁决就是上链的那一票。',
    burner: '一次性 EOA，经 EIP-7702 升级为智能账户——持有 0 ETH，签署 7710 bundle，所有费用只用 USDC 支付。',
    oneShot: '1Shot 无许可中继——垫付真实 ETH gas，在 Base 主网广播 castVote bundle，以 USDC 结算。',
